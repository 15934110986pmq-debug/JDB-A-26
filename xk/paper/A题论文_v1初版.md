...
最后，面向附件 3 + 附件 4 的多目标任务调度，建立 **滑动窗候选生成 + 分层目标 ILP (Hierarchical-Objective ILP) + chance constraint 鲁棒化** 的优化框架（M5）。在严格题面约束（射击 $d \in [5,30]$ m、$v \le 2$ m/s、校准 1.5 s；拍照 $d \in [10,40]$ m、$v \le 1.5$ m/s、对准 0.5 s、角度差 $\ge 60°$）下，scipy.milp + HiGHS（$n = 323$ 候选 + $G = 36$ 覆盖变量）求得**主解（标称）34 任务**（29 射击 + 5 拍照，期望命中 24.65）。**核心方法学贡献**：等权 $\max \sum x_i$ 因射击候选稠密（295 vs 28）给出"31 射击 + 4 拍照 = 35"失衡解；引入分层权重 $M_p = 1000 \gg M_s = 100 \gg 1$ 与覆盖指示变量 $y_g$，强制拍照覆盖优先，仅减 1 任务即使拍照覆盖 4 $\to$ 5——达**拍照硬上限 (Physical Photo Cap)**。该上限由题面"$v \le 1.5$ + 视角差 $60°$ + 持续 0.5 s"与轨迹低速段（仅 $[445, 510]$ s）乘积所决定，经独立 ILP 验证，非模型缺陷。鲁棒解（chance constraint，$z = 1.645$）31 任务（27 射击 + 4 拍照）作为 90% 置信工程对照。
...
$$\max_{\{x_i\},\{y_g\}}\; M_p\!\!\sum_{g \in \mathcal G_{\text{photo}}}\!\!y_g \;+\; M_s\!\!\sum_{g \in \mathcal G_{\text{shoot}}}\!\!y_g \;+\; \sum_{i=1}^n x_i,$$

s.t. 覆盖指示 $y_g \le \sum_{i:\text{cand}_i=g} x_i$；时段不冲突 $x_i + x_j \le 1$（$t_{\text{exec},i} + \epsilon > t_{\text{prep},j}$）；射击同目标 $\sum_k x_k \le K = 3$；$x_i, y_g \in \{0,1\}$。分层权重 $M_p = 1000 \gg M_s = 100 \gg 1$，覆盖指示变量 $y_g$ 共 $G = 36$ 个。

#### 5.5.2 算法设计

scipy.optimize.milp + HiGHS。$n = 323$ 候选 + $G = 36$ 覆盖变量 + $\sim$5000 时段冲突约束，亚秒级收敛（ILP 求解状态 Optimal）。双解对照：标称（无鲁棒，$\epsilon = 0$）+ 鲁棒（chance constraint，$z = 1.645$，$\epsilon = 0.1$ s）。**等权 ILP 失衡动机**：等权 $\max \sum x_i$ 在射击候选 295 vs 拍照候选 28 的悬殊规模下给出"31 射击 + 4 拍照 = 35"失衡解，拍照未达物理上限 5；引入分层目标修复覆盖均衡，是本模型的核心算法改进。

#### 5.5.3 求解结果

| 维度 | **标称解（主交付）** | 鲁棒解（对照） |
|---|---|---|
| 候选总数 | **323** | 275（剔除 48 个边界候选） |
| 入选任务 | **34** | 31 |
| 射击 + 拍照 | **29 + 5** | 27 + 4 |
| 期望命中数 | $29 \times 0.85 = \mathbf{24.65}$ | $27 \times 0.85 = 22.95$ |
| 拍照覆盖 | **5 / 18（达物理上限）** | 4 / 18 |
| 射击覆盖 | 15 / 18 | 14 / 18 |
| 时间跨度 | $[445.00, 808.80]$ s | — |
| ILP 求解状态 | Optimal（$-\text{obj} = 6534$） | Optimal |

**关键发现（等权 ILP 失衡 → 分层目标修复）**：等权 $\max \sum x_i$ 因射击候选稠密（295 vs 28）给出"31 射击 + 4 拍照"失衡解，拍照未达物理上限；引入分层权重 $M_p = 1000 \gg M_s = 100 \gg 1$ 后，**仅减 1 任务**（35 → 34）即使拍照覆盖 4 → 5（达物理上限）。拍照硬上限 $N_{\text{photo}}^{\max} = 5$ 经独立 ILP 验证——是题面"$v \le 1.5$ + 视角差 $60°$ + 持续 $0.5$ s"与轨迹低速段（$[445, 510]$ s）乘积的物理后果，非模型缺陷。详见 `xk/paper/Q4.md` §6.6。
...
| Q4 chance z 0 → 1.645 | **本文实测** | -3 任务（标称 34 → 鲁棒 31，剔除 48 边界候选） |
| Q4 ILP 等权 → 分层 | **本文实测** | -1 任务（35 → 34）但拍照覆盖 4 → 5（达物理上限） |
| Q4 K 1 → 3 | **本文实测** | 总数 20 → 34（拍照不变，射击补射） |
...
**(4) 不等方差 BLUE + 分层目标 ILP + chance constraint 的工程鲁棒化**：Q3 从 Q2 等方差自然推广到 $\sigma_1 \ne \sigma_2$；Q4 引入**分层目标 ILP**（$M_p = 1000 \gg M_s = 100 \gg 1$）解决等权 ILP 的覆盖失衡（31 射击 + 4 拍照 → 29 射击 + 5 拍照，仅减 1 任务即达拍照物理上限），并通过 chance constraint $d - z\sigma_d \ge d_{\min}$（$z = 1.645$）把 KF 不确定度纳入鲁棒对照解。
...
- 主解 34 任务（29 射击 + 5 拍照，期望命中 24.65）：拍照达物理上限 5/18，分层目标 ILP 以最小代价（仅减 1 任务）实现覆盖均衡；鲁棒解 31 任务（chance constraint，$z = 1.645$）作为 90% 置信工程对照，**拍照硬上限是题面约束与轨迹的物理后果，非模型缺陷**；
...
**(4) 分层目标 ILP 任务调度与覆盖物理上限（Hierarchical-Objective ILP Scheduling）**：Q4 主解（标称）34 任务（29 射击 + 5 拍照，期望命中 24.65），拍照达物理上限 5/18——经独立 ILP 验证，"$v \le 1.5$ + 视角差 $60°$ + 持续 0.5 s"与机器人轨迹低速段（仅 $[445, 510]$ s）的乘积将拍照机会窗压缩至 5 个互不冲突时段，是题面与轨迹共同决定的物理后果，非模型缺陷。分层目标 ILP（$M_p = 1000 \gg M_s = 100 \gg 1$）以仅减 1 任务（35 → 34）的代价使拍照覆盖从 4 升至 5（物理上限）；鲁棒解（chance constraint，$z = 1.645$）31 任务作为 90% 置信工程对照。
